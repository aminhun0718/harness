...
4. **간결하게 응답한다** — 맥락은 유지하되 분량은 최소. 서론/중복 요약/불필요한 리스트 금지. 설계 토론·의사결정 요청 때만 예외
5. {{프로젝트 특화 원칙 - Step 2 답변에서 도출}}
...
- 실기기 또는 시뮬레이터에서 테스트 (iOS + Android 양쪽 확인)
- 리스트 성능 확인 (FlatList 최적화, 불필요한 리렌더링)

### 2. 수정 및 커밋
- CRITICAL 이슈 해결 후 재검토
- 모든 검토 통과 후 커밋
