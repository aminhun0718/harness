...
## Codex 리뷰 프로토콜 (스코핑 + Dialog)

Codex는 Claude와 독립된 모델이라 **교차 검증 편향 축소** 목적으로 쓴다. 하지만 단방향 전수 리뷰는 **시간 낭비 + 거짓 양성**을 만든다. 이 프로토콜을 모든 Codex 리뷰에 적용한다.

### 핵심 원칙

1. **Scope: git diff 기본, 전수는 명시 이유**
   - 기본: `git diff main..HEAD` 또는 `git diff HEAD~1`로 바뀐 파일만 리뷰
   - 전수 리뷰(`Review all files`)는 다음 경우에만 허용:
     - 초기 하네스 세팅 직후 1회
     - 메이저 리팩터링 후
     - 명시적 보안 감사
   - 전수 리뷰 시에도 "전부"가 아니라 **핵심 10개 이내 파일로 제한**
   - **Diff 자체를 프롬프트에 포함**한다. 파일 경로만 주면 Codex가 전체 파일을 다시 읽어 시간·토큰 낭비

2. **Breadth: 관점 최대 2개**
   - 한 리뷰 요청에 관점(focus area) 2개까지만
   - "알고리즘 정확성 + 보안 우회" ✅ / "알고리즘 + 보안 + 타입 + 아키텍처" ❌
   - 관점이 3개 이상 필요하면 **리뷰를 2번 나눈다**
   - "놓친 위험", "기타 제안", "추가 발견사항" 같은 unbounded catch-all 섹션 **금지**

3. **Out-of-scope notes escape hatch**
   - Codex가 scope 밖에서 진짜 중요한 걸 발견했을 때 포맷을 깨지 않도록 "Out-of-scope notes" 섹션 허용
   - **최대 3줄**. 그 이상은 다음 리뷰로.

4. **Effort: 기본 unset**
   - `--effort` 플래그 기본 미지정 (Codex 판단)
   - `high`: 알고리즘 정확성, 보안 게이트 같은 진짜 중요한 것만
   - `low`: 빠른 스모크 체크 (diff 5줄 수준)

5. **출력 포맷 (구조화)**
   - 🔴 CRITICAL / 🟡 WARNING / 🟢 OK 분류
   - 각 발견: 파일 경로 + 라인 번호 + 1~2문장 근거 + 1문장 개선안
   - 관점별 섹션으로 그룹화
   - 끝에 "Top N Fixes" (N은 관점 수와 동일 = 1~2)
   - 전체 길이 목표: 2000 토큰 이내
   - 허식/칭찬 금지

### Dialog Mode (1+1: 기본 1턴, 반박 시 Turn 2 자동 추가, 최대 2턴)

단방향 리뷰는 Codex가 맥락 없이 잘못 짚은 발견도 그대로 리포트에 남긴다.
Claude가 맥락을 아는 경우 **Turn 2로 반박**하여 거짓 양성을 걸러낸다.
...
Turn 1 응답 — Codex → Claude
  [구조화된 발견 반환]

[Claude 판단: Turn 2 필요한가?]

Turn 2 필요 → Claude → Codex (--resume)
  [특정 발견에 대한 반박/재확인 요청]

Turn 2 응답 — Codex → Claude
  [수정된 판단 또는 의견 유지]

[자동 종료. 결과 정리하여 사용자에게 보고.]
```

**Turn 2 발동 기준** (둘 중 하나라도 해당하면):

- Codex 발견 중 **사실관계 오류**가 있는 경우 — Claude가 문맥/파일을 알기 때문에 Codex가 모르고 critical로 올린 것
- Codex 발견이 **전제된 scope를 벗어났거나**, 답변의 **핵심 근거가 모호**해서 파일:라인 재확인이 필요한 경우

**Turn 2 금지** (기본 1턴으로 종료):

- Codex 발견에 Claude가 **전부 동의**하는 경우
- 단순 등급 차이(🔴 vs 🟡) — 이건 사용자 판단 영역이라 그대로 전달
- Codex가 이미 "partial confirm" 같은 nuanced 답을 준 경우
- **반박할 게 없으면 Turn 2 하지 않는다**. 기본은 1턴.

**Turn 2 제한**:

- 한 리뷰당 **최대 1회 Turn 2** (= 총 2턴까지. 3턴 이상 금지)
- 2턴 이후에도 불일치가 남으면 "사용자 결정 영역"으로 분류하여 보고
...
Turn 1에서 당신이 X라 했는데 다음 맥락이 있음:
- [파일:라인 혹은 맥락 근거]
- [Claude가 아는 프로젝트 상태]

질문 1개만: [재검토 요청] 또는 [당신 의견 유지?]
답변은 1~3줄로 간결히.
```
...
## ❌ 논쟁 후 기각 (Claude 반박, Codex 수용)
- 원래 Codex 발견: ...
- Claude 반박 근거: ...
- Codex 수정: 기각 또는 등급 하향

## ⚠️ 이견 (두 모델 불일치 → 사용자 결정 영역)
- Codex 주장: ...
- Claude 주장: ...
- 사용자가 판단할 것: ...
...
---
<details>
<summary>Turn 2 대화 기록 (투명성)</summary>
Claude → Codex: ...
Codex → Claude: ...
</details>
```

단방향(1턴)으로 끝나는 경우는 기존 출력 포맷 그대로 전달한다 — "논쟁 후 기각", "이견" 섹션 생략.
...
| 증상 | 대응 |
|------|------|
| 같은 `progressPreview`가 5분 이상 멈춤 | 좀비 가능성 → status 로그 확인 후 cancel. 재시도 시 **--fresh** |
| `thread not found` 에러 | shared runtime pipe 끊김. cancel 후 재시도 (자동 좀비 정리는 companion이 못함) |
| `Command declined` (PowerShell 차단) | 파일 읽기 도구 차단됨. 프롬프트에 파일 수 줄이거나 diff 내용을 직접 포함 |
| 19분+ 정상 phase 지속 | 전수 리뷰일 가능성 크다. 취소하고 **스코프 축소**하여 재시도 |

**시간 가이드라인**:
- 1턴 타겟 리뷰(파일 5~10개, 관점 2개): **2~4분**
- 5분 넘어가면 진행 상태 확인, 8분 넘어가면 좀비 가능성 평가
- 전수 리뷰: 5~10분 예상, 15분 넘어가면 취소
